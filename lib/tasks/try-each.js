...
module.exports = CoreObject.extend({
  async run(scenarios, options) {
    ...
    let dependencyManagerAdapters = this.dependencyManagerAdapters || DependencyManagerAdapterFactory.generateFromConfig(this.config, this.project.root);
  ...
  async _runCommandForThisScenario(scenario) {
    ...
    let result = await this._runCommand({ commandArgs: command, commandOptions: this._commandOptions(scenario.env) });
